...
*/
   ...
   int max(int a, int b)
      ...
      public int getMinPel()
      {
         int min = 255;

         for (int iRow = 0; iRow < arrayOfPels.length; iRow++)
         {
            for (int iCol = 0; iCol < arrayOfPels[0].length; iCol++)
            {
               int v = arrayOfPels[iRow][iCol] & 0xFF; // extract grayscale
               if (v < min)
                  min = v;
            }
         }
